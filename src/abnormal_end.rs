...
mod abend_tests {
    ...
    #[test]
    fn fromraw_1() {
        let raw = RingItem::new(ring_items::ABNORMAL_END);
        let result: Option<AbnormalEnd> = raw.to_specific(RingVersion::V11);
        assert!(result.is_some());
    }
    #[test]
    fn fromraw_2() {
        let raw = RingItem::new(ring_items::BEGIN_RUN);
        let result: Option<AbnormalEnd> = raw.to_specific(RingVersion::V11);
    ...
    fn toraw_1() {
        ...
        let raw = end.to_raw();
        assert_eq!(ring_items::ABNORMAL_END, raw.type_id());
